...
string Printer::PrintQualifiedName ( QualifiedName qName ) {
    ...
    if ( qName.name != TAO::String_Manager ( "" ) ) {
        output = "ns=";
...
string Printer::PrintLocalizedText ( LocalizedText lText ) {
    ...
    if ( lText.locale != TAO::String_Manager ( "" ) ) {
        output = "lo=";
        output += lText.locale;
        output += ";s=";
        output += lText.text;
...
string Printer::PrintExtensionObject ( ExtensionObject eObject ) {
    ...
    if ( eObject.body._d() == BodyEncoding::BYTESTRING_BODY_ENCODING ) {
        for ( int i = 0; i < eObject.body.bytestring_encoding().length(); i++ ) {
        ...
        for ( int i = 0; i < strlen ( eObject.body.xmlelement_encoding() ); i++ ) {
...
string Printer::PrintVariant ( const Variant & variant ) {
    ...
    if ( variant.array_dimensions.length() == 1 && variant.array_dimensions[0] == 1 ) {
    ...
    } else if ( variant.array_dimensions.length() == 1 && variant.array_dimensions[0] > 1 ) {
